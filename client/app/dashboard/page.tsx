...
const SentimentSummaryCard: React.FC<{ mentions: Mention[] }> = ({
  mentions,
}) => {
...
export default function Dashboard() {
  ...
  const filteredLiveMentions = useMemo(
    () => filterMentions(mentions, liveFeedFilter),
    [mentions, liveFeedFilter]
  );
  const filteredRedditMentions = useMemo(
    () => filterMentions(redditMentions, redditFilter),
    [redditMentions, redditFilter]
  );
  const filteredNewsMentions = useMemo(
    () => filterMentions(newsMentions, newsFilter),
    [newsMentions, newsFilter]
  );
  const filteredYoutubeMentions = useMemo(
    () => filterMentions(youtubeMentions, youtubeFilter),
    [youtubeMentions, youtubeFilter]
  );
  ...
  useEffect(() => {
    const loadMentions = async () => {
      try {
        ...
        setRedditMentions(
          allMentions.filter((m: Mention) =>
            normalizeSource(m.source).includes("reddit")
          )
        );
        setNewsMentions(
          allMentions.filter(
            (m: Mention) =>
              normalizeSource(m.source).includes("gnews") ||
              normalizeSource(m.source).includes("news")
          )
        );
        setYoutubeMentions(
          allMentions.filter((m: Mention) =>
            normalizeSource(m.source).includes("youtube")
          )
        );
      } catch (error) {
  ...
  const handleManualRefresh = async () => {
    ...
    try {
      await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/refresh-feed`
      );
    } catch (err) {
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          size="icon"
          className={`relative w-10 bg-blue-50 border-blue-400 h-10 ${
            activeAlerts.length > 0 ? "border-red-500 hover:bg-red-50/50" : ""
          }`}
        >
        ...
        <div className="p-6 pt-4 flex-grow overflow-y-auto">
          ...
          {activeAlerts.length > 0 ? (
            ...
            <p className="text-center text-gray-500 pt-10">
              No active spike alerts. All clear!
            </p>
          )}
        </div>
      </DialogContent>
  ...
  const SourceFeedCard: React.FC<{
    ...
    return (
      <Card className="shadow-lg dark:bg-gray-900/50">
        ...
        <CardContent
          className={`${FEED_HEIGHT_CLASS} overflow-y-auto space-y-4 pt-2`}
        >
          {mentions.map((m, i) => (
            <MentionCard
              key={i}
  ...
  return (
    <div className="p-4 md:p-4 -pt-3 lg:p-8 mx-auto container space-y-8 bg-white dark:bg-gray-950 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-4 dark:border-gray-800">
        ...
        <div className="flex flex-wrap gap-3 md:space-x-2 items-center">
          {" "}
          {/* Notification Modal */}
          <NotificationModal
          ...
          >
            ...
            <span className="text-xs text-gray-400">{" (Default: 5min)"}</span>
          </Button>
          <Button
            variant="secondary"
        ...
        <Card className="lg:col-span-1 xl:col-span-2 shadow-lg dark:bg-gray-900/50">
          <CardHeader className="pb-4 flex flex-row justify-between items-center">
            ...
            <div className="w-full max-w-[200px]">
              <Select
                onValueChange={(value) =>
                  setLiveFeedFilter(value as FilterType)
                }
