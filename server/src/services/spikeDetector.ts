// server/src/services/spikeDetector.ts
...
export const trackMention = () => {
    ...
    // 1. Add the new mention
    mentionTimestamps.push({ timestamp: now });

    // 2. Clean up (remove timestamps older than the LONG_WINDOW_MS)
    const cutoffTime = now - LONG_WINDOW_MS;
    mentionTimestamps = mentionTimestamps.filter(m => m.timestamp >= cutoffTime);
};

/**
 * Checks for a conversation spike in the last SHORT_WINDOW.
 * @returns {boolean} True if a spike is detected.
 */
export const checkForSpike = (): boolean => {
    ...
    const shortWindowMentions = mentionTimestamps.filter(m => m.timestamp >= shortWindowCutoff);
    ...
    if (shortCount > averageRatePerShortWindow * SPIKE_THRESHOLD_MULTIPLIER) {
        console.log(`🔥 SPIKE DETECTED! Short Count: ${shortCount}, Baseline Avg: ${averageRatePerShortWindow.toFixed(2)}`);
        return true;
    }
